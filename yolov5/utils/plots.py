...
class Annotator:
    ...
    def centerpointbbox(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        x_point, y_point = int((box[0] + box[2]) / 2), int((box[1] + box[3]) / 2)
        cv2.circle(self.im, (x_point, y_point), 1, color, -1)
    ...
    def listsendupdate(self,idlist,segpo, color=(128, 128, 128), txt_color=(255, 255, 255)):
        r = random.randint(0,255)
        g = random.randint(0,255)
        b = random.randint(0,255)
        pobox = idlist[0:4]
        idint = int(idlist[4])-1
        # x_point, y_point = int((pobox[0] + pobox[2]) / 2), int((pobox[1] + pobox[3]) / 2)
        _x_list = [vertex [0] for vertex in segpo]
        _y_list = [vertex [1] for vertex in segpo]
        _len = len(segpo)
        x_point = int(sum(_x_list) / _len)
        y_point = int(sum(_y_list) / _len)
        idpotp = (x_point,y_point)
        if y_point < 100:
            try:
                listpo[idint]=idpotp
            except IndexError:
                listpo.insert(idint,idpotp)
            print(listpo) 
            fristpo = listpo[ipo[0]]
            if len(listpo) != ipo[0]+1:
                if cv2.waitKey(2) == ord('r'):
                    ipoint = ipo[0] + 1
                    ipo.insert(0,ipoint)
            elif len(listpo) == ipo[0]+1:
                pass
            cv2.circle(self.im,(fristpo[0],fristpo[1]) , 6, (r,g,b), -1)
            xfrist,yfrist = fristpo[0],fristpo[1]
            return xfrist,yfrist
        else:
            pass
            return 0,0
